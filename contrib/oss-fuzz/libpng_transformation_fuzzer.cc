...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  */
  ...
  It literally says in the comment "it's tricky", so maybe there're bugs out there?
  */
  ...
  // actually, can potentially "append" data beyond .png image format, without breaking the ability of common png viewers to display the image properly.
  // tested with cat 'abc' >> img.png
  // so no need to store that fuzzing data in EXIF / TXT chunks...
  // It would cause, however too much branching, so desided to stick to const definitions
